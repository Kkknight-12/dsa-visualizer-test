# DSA Visualizer & Problem Progress Tracker 📈

This file tracks the step-by-step implementation of all Striver A2Z Sheet problems, user approach selections, dry-run code implementations, and interactive visualizers.

---

## 📌 Master Implementation Status

| # | LeetCode # | Problem Name | Category | `problem.md` | Chosen Approach | `.ts` Implementation | 2D Visualizer | User Feedback / Status |
|---|------------|--------------|----------|--------------|-----------------|----------------------|---------------|------------------------|
| 1 | LC 145 | Binary Tree Postorder Traversal | Trees | ✅ Done | Optimal (Iterative 2-Phase Stack) | `PostorderTraversal.ts` | ✅ Done (`/postorder-3d`) | ⭐ Approved (Shiki Line Auto-Centering + Multi-Layout Studio) |
| 2 | SYS-CH | Consistent Hashing | System Design | ✅ Done | Hash Ring & Virtual Nodes | `ConsistentHashing.ts` | ✅ Done (`/consistent-hashing`) | ⭐ Approved |
| 3 | **LC 75** | **Sort Colors (0s, 1s, 2s)** | **Arrays** | ✅ `problem-sort-colors.md` | Pending User Choice | Pending | In Queue | **Awaiting User Choice** |
| 4 | LC 53 | Maximum Subarray (Kadane's) | Arrays | ⏳ Queued | - | - | In Queue | Next |
| 5 | LC 42 | Trapping Rain Water | Arrays | ⏳ Queued | - | - | In Queue | Next |
| 6 | LC 33 | Search in Rotated Sorted Array | Binary Search | ⏳ Queued | - | - | In Queue | Next |
| 7 | LC 206 | Reverse Linked List | Linked List | ⏳ Queued | - | - | In Queue | Next |
| 8 | LC 496 | Next Greater Element I | Stacks & Queues | ⏳ Queued | - | - | In Queue | Next |
| 9 | LC 743 | Network Delay Time (Dijkstra) | Graphs | ⏳ Queued | - | - | In Queue | Next |
| 10 | LC 1143 | Longest Common Subsequence (LCS) | DP | ⏳ Queued | - | - | In Queue | Next |

---

## 📝 Active Problem Log: LC 75 — Sort Colors (0s, 1s, 2s)

- **Step 1**: Create `problem-sort-colors.md` explaining Brute Force, Better Approach, and Optimal Dutch National Flag (3 Pointers) in Hinglish.
- **Step 2**: Solicit user choice for which approach to implement in TypeScript (`.ts`) and 2D Visualizer.
