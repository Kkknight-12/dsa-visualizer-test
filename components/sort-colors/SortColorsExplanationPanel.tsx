...
interface SortColorsExplanationPanelProps {
  currentStep: SortColorsStep;
  totalSteps: number;
}

export function SortColorsExplanationPanel({ currentStep, totalSteps }: SortColorsExplanationPanelProps) {
  const {
    stepNumber,
    activeLine,
    actionTitle,
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-400 font-mono mt-0.5">
              Detailed breakdown of Dutch National Flag 3-pointer partition logic
            </p>
          </div>
        </div>
            ...
            <div>
              <div className="flex items-center gap-2.5 flex-wrap">
                <span className="font-mono text-base font-black text-amber-300">
                  FINAL RESULT: Fully Sorted Array = [{arraySnapshot.map((e) => e.val).join(', ')}]
                </span>
                <span className="text-xs sm:text-sm font-mono px-3 py-0.5 rounded-md bg-amber-500/25 text-amber-300 border border-amber-500/40 font-bold">
                  In-Place Partitioned
                </span>
              </div>
              <p className="text-sm text-slate-200 font-sans mt-1">
                Virtual Boundaries: <span className="font-mono font-bold text-rose-300">0s [0...{low - 1}]</span> • <span className="font-mono font-bold text-slate-200">1s [{low}...{mid - 1}]</span> • <span className="font-mono font-bold text-sky-300">2s [{high + 1}...{arraySnapshot.length - 1}]</span>
              </p>
            </div>
          </div>
