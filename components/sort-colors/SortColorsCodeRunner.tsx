...
interface SortColorsCodeRunnerProps {
  currentStep: SortColorsStep;
  totalSteps: number;
}

const RAW_CODE = `function sortColors(nums: number[]): void {
  let low = 0;
  let mid = 0;
  ...
  while (mid <= high) {
    if (nums[mid] === 0) {
      // Case 0: Swap with low region
      [nums[low], nums[mid]] = [nums[mid], nums[low]];
      low++;
      mid++;
    } else if (nums[mid] === 1) {
      // Case 1: Already in middle region
      mid++;
    } else {
      // Case 2: Swap with high region
...
export function SortColorsCodeRunner({ currentStep, totalSteps }: SortColorsCodeRunnerProps) {
