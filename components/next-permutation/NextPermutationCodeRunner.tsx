...
interface NextPermutationCodeRunnerProps {
  currentStep: NextPermutationStep;
  totalSteps: number;
}

const RAW_CODE = `function nextPermutation(nums: number[]): void {
  const n = nums.length;
  if (n <= 1) return;

  // STEP 1: Find breakpoint index i (nums[i] < nums[i + 1])
  let i = n - 2;
  while (i >= 0 && nums[i] >= nums[i + 1]) {
    i--;
  }

  // STEP 2: Find rightmost j (nums[j] > nums[i]) & swap
  if (i >= 0) {
    let j = n - 1;
  ...
  // STEP 3: Reverse right suffix (i + 1 to n - 1)
...
export function NextPermutationCodeRunner({ currentStep, totalSteps }: NextPermutationCodeRunnerProps) {
  ...
  return (
    <div className="w-full h-full bg-[#0d1117] border border-slate-800 rounded-2xl p-4 sm:p-5 shadow-2xl backdrop-blur-xl flex flex-col justify-between font-mono text-sm overflow-hidden">
      ...
      <div
        ...
        className="relative flex-1 overflow-x-auto overflow-y-auto max-h-[380px] min-h-[280px] bg-[#16161e] p-3.5 rounded-xl border border-slate-800/90 space-y-0.5 select-text"
      >
          ...
          return (
            ...
            >
              <div className="flex items-center font-mono">
                {/* Gutter Line Number + Laser Pointer */}
